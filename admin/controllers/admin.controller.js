...
import products from "../../src/model/products.model.js";
import auth from "../../account/model/auth.model.js"
import orders from "../../src/model/orders.model.js"
...
export default new class Admin {
    ...
    async adminEnter(req, res) {
        try {
            ...
            const admin = await adminModel.find({ name });
            if (!admin ) {
              res.redirect('/');
            } else {
         ...
         const orderStats = await orders.aggregate([
            {
              $group: {
                _id: null,
                totalSum: { $sum: {
                    $toDouble: { $convert: { input: "$total", to: "double" } }
                }}
              }
            }
          ]);
